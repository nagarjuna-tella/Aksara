"""
Aksara AI Runtime (Mini Agent Loop) - v0.4.6

A minimal "agent runtime" inside Aksara that coordinates:
...
can use to drive Aksara.

The flow:
1. External AI sends an AgentIntent
2. Aksara returns an AgentContextBundle (full context + schemas)
...
class AgentIntent(BaseModel):
    ...
    sends this to Aksara to get context and schemas for planning.
...
class AgentContextBundle(BaseModel):
    ...
    - Understand the Aksara application structure
    ...
    version: str = Field(..., description="Aksara framework version")
...
async def build_agent_context_bundle(
    ...
    from aksara.ai.planner import AiPlan, AiPlanStep, get_plan_schema
    from aksara.ai.patch import AiPatchRequest, AiPatchOperation
