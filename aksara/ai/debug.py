...
class AiEnvInfo(BaseModel):
    ...
    aksara_version: str = Field(..., description="Aksara framework version")
    ...
    installed_apps: list[str] = Field(default_factory=list, description="Installed Aksara apps")
    ...
    def _analyze_db_error(self, context: AiDebugContext) -> list[AiDebugSuggestion]:
        ...
        if "does not exist" in msg or "no such table" in msg:
            suggestions.append(AiDebugSuggestion(
                ...
                code_snippet="python -m aksara migrate",
                confidence=0.95,
                category="database",
            ))
        ...
        if "column" in msg and ("does not exist" in msg or "unknown" in msg):
            suggestions.append(AiDebugSuggestion(
                ...
                code_snippet="python -m aksara makemigrations\npython -m aksara migrate",
                confidence=0.9,
                category="database",
            ))
...
async def build_ai_debug_context(
    ...
    if debug_context:
        ...
        try:
            from aksara.middleware.context import request_id_var, tenant_id_var
