"""
AI CodeGen Helpers for Aksara.

This module provides deterministic code generation from structured specs.
LLMs produce AiModelSpec/AiCodegenRequest JSON, Aksara generates real code.

The design is provider-agnostic: no LLM calls happen here.
We simply transform structured specs into Aksara-compatible code artifacts.
...
def _get_quote_identifier():
    """Lazy import quote_identifier to avoid importing DB helpers at module load time."""
...
def _validate_identifier(name: str, kind: str) -> str:
    """Validate a SQL identifier against the framework allowlist."""
...
class AiModelSpec(BaseModel):
    """
    Specification for generating a Aksara model.
...
def generate_model_code(spec: AiModelSpec) -> Dict[str, str]:
    """
    Generate Python code for a Aksara model.
    ...
    lines = [
        ...
        f'{spec.name} Model',
        '',
        f'Generated by Aksara AI CodeGen at {datetime.now().isoformat()}',
        '"""',
...
def generate_viewset_code(spec: AiModelSpec) -> Dict[str, str]:
    """
    Generate Python code for a Aksara ModelViewSet.
...
def generate_serializer_code(spec: AiModelSpec) -> Dict[str, str]:
    """
    Generate Python code for a Aksara ModelSerializer.
...
def generate_admin_code(spec: AiModelSpec) -> Dict[str, str]:
    ...
    lines = [
        ...
        f'Generated by Aksara AI CodeGen at {datetime.now().isoformat()}',
        '"""',
        'from aksara.contrib.admin import ModelAdmin, register',
        f'from {spec.app_label}.models import {spec.name}',
        '',
        '',
...
def generate_app_skeleton(app_name: str) -> Dict[str, str]:
    """
    Generate a basic Aksara app skeleton.
...
Generated by Aksara AI CodeGen at {datetime.now().isoformat()}
"""
'''
...
def generate_migration_stub(spec: AiModelSpec) -> Dict[str, str]:
    ...
    quote_identifier = _get_quote_identifier()
    table_name = _validate_identifier(
        spec.table_name or f"{spec.app_label}_{spec.name.lower()}",
        "table",
    )
    
    # Build column definitions
    columns = ['        (quote_identifier("id"), "SERIAL PRIMARY KEY")']
    for field in spec.fields:
        field_name = _validate_identifier(field.name, "field")
        col_type = _get_sql_type(field)
        nullable = "" if field.required else " NULL"
        unique = " UNIQUE" if field.unique else ""
        columns.append(
            f'        (quote_identifier("{field_name}"), "{col_type}{nullable}{unique}")'
        )
    ...
    lines = [
        ...
        columns_str,
        '    ]',
        f'    table_name = "{quoted_table_name}"',
        '    ',
        ...
        f'    await db.execute("DROP TABLE IF EXISTS {quoted_table_name}")',
        '',
    ]
...
def generate_code(request: AiCodegenRequest) -> AiCodegenResult:
    """
    Generate Aksara code from a structured request.
    ...
    if target == AiCodegenTarget.MODEL:
        ...
        notes.append(f"Run 'aksara makemigrations --app {spec.app_label}.models' to create migrations")
        notes.append(f"Run 'aksara migrate --app {spec.app_label}.models' to apply migrations")
        ...
        notes.append("Consider using 'aksara makemigrations' for production migrations")
