...
Structured logging for Aksara ORM with debug mode support.
"""
...
# Create Aksara logger
logger = logging.getLogger("aksara")

# Default format for Aksara logs
DEFAULT_FORMAT = "[AKSARA] %(message)s"


def setup_logging(level: int = logging.DEBUG, format_string: Optional[str] = None) -> None:
    """
    Set up logging for Aksara.
    
    Call this if you want Aksara to configure its own handler.
...
def get_logger(name: Optional[str] = None) -> logging.Logger:
    ...
    Args:
        name: Optional sub-logger name (e.g., "aksara.db", "aksara.query")
        
    Returns:
        Logger instance
    """
    if name:
        return logging.getLogger(f"aksara.{name}")
    return logger
