"""Verify the v0.7.2 final evidence manifest and credential boundary."""
...
EVIDENCE = ROOT / "audit-evidence/v072-final"
PROTECTED = (
...
def main() -> None:
    ...
    for path in EVIDENCE.iterdir():
        if path.is_file() and credential_pattern.search(path.read_bytes()) is not None:
            raise SystemExit(
                f"credential-like database URL in {path.relative_to(ROOT)}"
            )
