...
def build_shell_namespace(
    ...
    Creates a namespace with commonly used Vidyut imports and
...
def get_shell_banner(namespace: dict[str, Any]) -> str:
    """
    Generate a welcome banner for the Vidyut shell.
    ...
    lines = [
        "",
        f"  \033[33m⚡\033[0m \033[1mVidyut Shell\033[0m v{__version__}",
        "",
        "  Available objects:",
        "    • Model, fields, Database, ModelRegistry",
        "    • settings, configure",
...
def start_ipython_shell(namespace: dict[str, Any], banner: str) -> None:
    ...
    # Custom prompts for Vidyut
    class VidyutPrompts(Prompts):
        def in_prompt_tokens(self, cli=None):
            return [(Token.Prompt, "vidyut> ")]
    ...
    # Configure IPython
    config = Config()
    config.TerminalInteractiveShell.prompts_class = VidyutPrompts
...
def run_shell(
    ...
    """
    Run the Vidyut interactive shell.
    
    Starts an interactive Python shell with Vidyut imports pre-loaded,
