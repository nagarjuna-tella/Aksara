...
This example contains `User`, `Post` and `Article`, serializers and generated REST routes.
...
aksara migrate --migrations-dir examples/basic_app/migrations
aksara run examples.basic_app.main:app --host 127.0.0.1 --port 8000
```
...
README's unauthenticated POST to `/api/users/` returns **403** with v0.7.0;
it does not create a record. Do not disable permission checks to make it pass.
The custom lifespan also attempts table creation for User and Post and suppresses its errors. This is historical demo behavior, not the production migration pattern. Manual endpoints coexist with generated ViewSets; do not assume identical authorization.
